...
public class LightningEditor {
				...
				if (tempLine.contains("}")) {
					...
					if (tempLine.contains("=")) {
						...
						if (lines.get(1).contains("{")) {
							...
							throw new LightningFileException("Error at '" + file.getAbsolutePath() + "' -> '" + tempLine
	...
	// <Read Data>
	private Map<String, Object> internalRead(final String filePath, final List<String> lines, int blankLine,
	                                         final int commentLine) throws ArrayIndexOutOfBoundsException {
		final Map<String, Object> tempMap = new HashMap<>();
		String tempKey = null;
		...
		while (lines.size() > 0) {
			...
			if (tempLine.equals("}")) {
				...
				throw new LightningFileException("Error at '" + filePath + "' -> Block closed without being opened");
			} else if (tempLine.isEmpty()) {
				blankLine++;
				tempMap.put("{=}emptyline" + blankLine, LineType.BLANK_LINE);
			} else if (tempLine.startsWith("#")) {
				...
				if (tempLine.contains("=")) {
					...
					if (lines.get(1).contains("{")) {
						...
						throw new LightningFileException(
								"Error at '" + filePath + "' -> '" + tempLine + "' does not contain value or subblock");
					}
				}
			}
		}
	...
	private void topLayerWriteWithComments(final PrintWriter writer, final Map<String, Object> map,
	...
	private void internalWriteWithComments(final Map<String, Object> map, final String indentationString,
		...
		for (final String localKey : map.keySet()) {
			...
			if (localKey.startsWith("#") && map.get(localKey) == LineType.COMMENT) {
				...
				if (map.get(localKey) instanceof Map) {
					...
					internalWriteWithComments((Map<String, Object>) map.get(localKey), indentationString + "  ",
							writer);
	...
	private void topLayerWriteWithOutComments(final PrintWriter writer, final Map<String, Object> map,
	                                          final String localKey) {
		if (!localKey.startsWith("#") && map.get(localKey) != LineType.COMMENT && !localKey.startsWith("{=}emptyline")
	...
	private void internalWriteWithoutComments(final Map<String, Object> map, final String indentationString,
	                                          final PrintWriter writer) {
		for (final String localKey : map.keySet()) {
			writer.println();
			if (!localKey.startsWith("#") && map.get(localKey) != LineType.COMMENT
					&& !localKey.startsWith("{=}emptyline") && map.get(localKey) != LineType.BLANK_LINE) {
				if (map.get(localKey) instanceof Map) {
					writer.print(indentationString + "  " + localKey + " " + "{");
					internalWriteWithoutComments((Map<String, Object>) map.get(localKey), indentationString + "  ",
							writer);
				} else if (map.get(localKey) instanceof List) {
					writer.println(indentationString + "  " + localKey + " = [");
					writeList((List<String>) map.get(localKey), indentationString + "  ", writer);
