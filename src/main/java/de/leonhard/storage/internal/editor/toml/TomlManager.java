...
public class TomlManager {
	...
	/**
	 * Writes data to a File, in the TOML format and with the UTF-8 encoding. The
	 * default indentation parameters are used, ie each indent is one tab character.
	 *
	 * @param data the data to write
	...
	/**
	 * Writes data to an OutputStream, in the TOML format and with the UTF-8
	 * encoding. The default indentation parameters are used, ie each indent is one
	 * tab character.
	 *
	 * @param data the data to write
	...
	/**
	 * Writes data to a Writer, in the TOML format and with the default parameters,
	 * ie each indent is 1 tab character. This is the same as
	...
	/**
	 * Writes the specified data to a Writer, in the TOML format and with the
	 * specified parameters.
	 *
	 * @param data             the data to write
	 * @param writer           where to write the data
	 * @param indentSize       the indentation size, ie the number of times the
	 *                         indentation character is repeated in one indent.
	 * @param indentWithSpaces true to indent with spaces, false to indent with tabs
	 * @throws IOException   if a read error occurs
	 * @throws TomlException if a parse error occurs
	 */
	public void write(final Map<String, Object> data, final Writer writer, final int indentSize,
	                  final boolean indentWithSpaces) throws IOException {
	...
	/**
	 * Reads a String that contains TOML data. Lenient bare keys are allowed (see
	 * {@link TomlManager}).
	 *
	 ...
	 *
	 * @param toml                a String containing TOML data
	 * @param strictAsciiBareKeys <code>true</code> to enforce strict bare keys (see
	 *                            {@link TomlManager}).
	 * @return a {@code Map<String, Object>} containing the parsed data
	 * @throws TomlException if a parse error occurs
	 */
	...
	/**
	 * Reads TOML data from an UTF-8 encoded File. Lenient bare keys are allowed
	 * (see {@link TomlManager}).
	 *
	...
	/**
	 * Reads TOML data from an UTF-8 encoded InputStream. Lenient bare keys are
	 * allowed (see {@link TomlManager}).
	 *
	...
	/**
	 * Reads TOML data from a Reader. The data is read until the end of the stream
	 * is reached.
	 *
	 * @param bufferSize          the initial size of the internal buffer that will
	 ...
	 * @throws TomlException if a parse error occurs
	 */
	public Map<String, Object> read(final Reader reader, final int bufferSize, final boolean strictAsciiBareKeys)
			throws IOException, TomlException {
